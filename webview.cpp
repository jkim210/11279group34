...
public:
    WebApp() :
        m_url("/Users/duckynatalie/11279group34/11279group34/demo/foliumMap.html") // TODO Change path
    {
    }
...
public:
    ...
    void OnHighlight(wxCommandEvent& evt);
...
private:
    ...
    wxToolBarToolBase* m_toolbar_erase;
    wxToolBarToolBase* m_toolbar_highlight;
    ...
    wxFrame(NULL, wxID_ANY, "wxWebView Sample")
{
    ...
    #ifdef __WXGTK__
        wxBitmap eraser = wxArtProvider::GetBitmap("gtk-eraser", wxART_TOOLBAR);
    #else
        wxBitmap eraser = wxBitmap(stop_xpm);
    #endif
    #ifdef __WXGTK__
        wxBitmap highlighter = wxArtProvider::GetBitmap("gtk-highlighter", wxART_TOOLBAR);
    #else
        wxBitmap highlighter = wxBitmap(refresh_xpm);
    #endif

    m_toolbar_back = m_toolbar->AddTool(wxID_ANY, _("Back"), back);
    m_toolbar_forward = m_toolbar->AddTool(wxID_ANY, _("Forward"), forward);
    m_toolbar_erase = m_toolbar->AddTool(wxID_ANY, _("Erase"), eraser);
    m_toolbar_highlight = m_toolbar->AddTool(wxID_ANY, _("Highlight"),  highlighter);
    ...
    m_toolbar_tools = m_toolbar->AddTool(wxID_ANY, _("Run Simulation"), wxBitmap(wxlogo_xpm));
    ...
    Bind(wxEVT_TOOL, &WebFrame::OnErase, this, m_toolbar_erase->GetId());
    Bind(wxEVT_TOOL, &WebFrame::OnHighlight, this, m_toolbar_highlight->GetId());
...
/**
  * Callback invoked when user pressed the "erase" button
  */
void WebFrame::OnErase(wxCommandEvent& WXUNUSED(evt))
{
    //m_browser->Stop(); // Old code
    // We code eraser functionality here!!!
...
/**
  * Callback invoked when user pressed the "highlight" button
  */
void WebFrame::OnHighlight(wxCommandEvent& WXUNUSED(evt))
{
    //m_browser->Highlight(); // Old code
    // We code highlighter functionality here!!!
...
void WebFrame::OnNavigationRequest(wxWebViewEvent& evt)
{
    ...
    {
        evt.Veto();
        m_toolbar->EnableTool( m_toolbar_erase->GetId(), false );
    }
    else
    {
        UpdateState();
